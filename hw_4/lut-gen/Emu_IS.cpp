...
Vec3f ImportanceSampleGGX(Vec2f Xi, Vec3f N, float roughness) {
    ...
    Vec3f up = abs(N.z) < 1.0 ? Vec3f(0.0, 0.0, 1.0) : Vec3f(1.0, 0.0, 0.0);
...
Vec3f IntegrateBRDF(Vec3f V, float roughness) {
    ...
    return Vec3f(A / sample_count, B / sample_count, 0.0f);
}
...
int main() {
    ...
    for (int i = 0; i < resolution; i++) {
        for (int j = 0; j < resolution; j++) {
            ...
            //data[(i * resolution + j) * 3 + 0] = uint8_t((1.0 - irr.x) * 255.0);
            //data[(i * resolution + j) * 3 + 1] = uint8_t((1.0 - irr.y) * 255.0);
            //data[(i * resolution + j) * 3 + 2] = uint8_t((1.0 - irr.z) * 255.0);
        }
    }
    stbi_flip_vertically_on_write(true);
    //stbi_flip_vertically_on_write(false);
    stbi_write_png("GGX_E_IS_LUT.png", resolution, resolution, 3, data, resolution * 3);
