        ...
        for (int i = 0; i < 6; i++)
        {
            for (int y = 0; y < height; y++)
            {
                for (int x = 0; x < width; x++)
                {
                    ...
                    Eigen::Vector3d dir_d(dir.x(), dir.y(), dir.z());
                    dir_d.normalize();

                    float u = 2 * ((x + 0.5) / width) - 1;
                    float v = 2 * ((y + 0.5) / height) - 1;
                    double area = CalcArea(u, v, width, height);

                    for (int l = 0; l <= SHOrder; l++) {
                        for (int m = -1 * l; m <= l; m++) {
                            int SHCoeffiecents_index = (l > 0 ? l * l : 0) + m + l;
                            SHCoeffiecents[SHCoeffiecents_index] += Le * sh::EvalSH(l, m, dir_d) * area;
                        }
                    }
                }
            }
...
public:
    ...
    void get_interreflection_sh(
        const Point3f& v,
        const Normal3f& n,
        const MatrixXf& m_TransportSHCoeffs_buffer,
        const Scene* scene,
        const int SHNum,
        const int bounce,
        int time,
        double e,
        std::unique_ptr<std::vector<double>>& inter_shCoeff) {
        if (time < bounce) {
            //Ray3f ray(v, n);
            //Intersection inter;
            //if (scene->rayIntersect(ray, inter)) {
            //    int p0_index = (int)inter.tri_index.x();
            //    int p1_index = (int)inter.tri_index.y();
            //    int p2_index = (int)inter.tri_index.z();

            //    Point3f insect_v = inter.bary.x() * inter.mesh->getVertexPositions().col(p0_index)
            //        + inter.bary.y() * inter.mesh->getVertexPositions().col(p1_index)
            //        + inter.bary.z() * inter.mesh->getVertexPositions().col(p2_index);

            //    Normal3f insect_n = inter.bary.x() * inter.mesh->getVertexNormals().col(p0_index)
            //        + inter.bary.y() * inter.mesh->getVertexNormals().col(p1_index)
            //        + inter.bary.z() * inter.mesh->getVertexNormals().col(p2_index);

            //    double H = insect_n.dot(-1.0 * n);
            //    if (H > 0.0) {
            //        e = e * H;
            //        for (int j = 0; j < SHNum; j++) {
            //            (*inter_shCoeff)[j] += (inter.bary.x() * m_TransportSHCoeffs_buffer.col(p0_index).coeffRef(j)
            //                + inter.bary.y() * m_TransportSHCoeffs_buffer.col(p1_index).coeffRef(j)
            //                + inter.bary.z() * m_TransportSHCoeffs_buffer.col(p2_index).coeffRef(j)) * e;
            //        }
            //        get_interreflection_sh(
            //            insect_v,
            //            insect_n,
            //            m_TransportSHCoeffs_buffer,
            //            scene,
            //            SHNum,
            //            bounce,
            //            time + 1,
    ...
    virtual void preprocess(const Scene *scene) override
    {
        ...
        for (int i = 0; i < mesh->getVertexCount(); i++)
        {
            ...
            auto shFunc = [&](double phi, double theta) -> double {
                ...
                else
                {
                    ...
                    if (H > 0) {
                        Ray3f ray(v, wi);
                        if (!scene->rayIntersect(ray)) return H;
                    }
        ...
        if (m_Type == Type::Interreflection)
        {
            ...
            MatrixXf m_TransportSHCoeffs_buffer(m_TransportSHCoeffs);
            constexpr int SHNum = (SHOrder + 1) * (SHOrder + 1);
            ...
            for (int i = 0; i < mesh->getVertexCount(); i++) {
                ...
                std::unique_ptr<std::vector<double>> inter_shCoeff(new std::vector<double>());
                inter_shCoeff->assign(SHNum, 0.0);
                ...
                for (int j = 0; j < SHNum; j++) {
                    m_TransportSHCoeffs.col(i).coeffRef(j) += 2 * PI * (*inter_shCoeff)[j] / (float)m_SampleCount;
                }
            }
        }
