...
class MeshRender {
	...
	bindMaterialParameters() {
		...
		for (let k in this.material.uniforms) {
			...
			else if (this.material.uniforms[k].type.includes("prtLight_")) {
				let index = parseInt(this.material.uniforms[k].type.replace("prtLight_", ""));
				gl.uniform3fv(
					this.shader.program.uniforms[k],
					precomputeL[guiParams.envmapId][index]
					//precomputeL_RGBMat3[index]
					);
			}
		}
	}
